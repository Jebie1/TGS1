...
document.getElementById('bisectionForm').addEventListener('submit', function(event) {
    ...
    do {
        ...
        stepsRow.innerHTML = `
            ...
            <td>
                Iteration ${iteration}:
                1. Calculate t = (a + b) / 2 = (${iteration === 1 ? a.toFixed(6) : currentA.toFixed(6)} + ${iteration === 1 ? b.toFixed(6) : currentB.toFixed(6)}) / 2 = ${t.toFixed(6)}
                2. Calculate f(a) = ${func.replace(/x/g, iteration === 1 ? a.toFixed(6) : currentA.toFixed(6))} = ${fA.toFixed(6)}
                3. Calculate f(b) = ${func.replace(/x/g, iteration === 1 ? b.toFixed(6) : currentB.toFixed(6))} = ${fB.toFixed(6)}
                4. Calculate f(t) = ${func.replace(/x/g, t.toFixed(6))} = ${fT.toFixed(6)}
                5. Check f(t) * f(a) = ${fT.toFixed(6)} * ${fA.toFixed(6)} = ${fTfA.toFixed(6)}
                6. Check f(t) * f(b) = ${fT.toFixed(6)} * ${fB.toFixed(6)} = ${fTfB.toFixed(6)}
                7. New Interval: ${decision}
            </td>
        `;
